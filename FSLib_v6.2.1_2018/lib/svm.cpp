...
private:
	...
	{   return dot(x[i],x[j])  +(i==j)*balanced_ridges[i];
	}
	double kernel_poly(int i, int j) const
	{
		return pow(gamma*dot(x[i],x[j])+coef0,degree)+(i==j)*balanced_ridges[i];
	}
	double kernel_rbf(int i, int j) const
	{
		return exp(-gamma*(x_square[i]+x_square[j]-2*dot(x[i],x[j])))+(i==j)*balanced_ridges[i];
	}
	double kernel_sigmoid(int i, int j) const
	{
		return tanh(gamma*dot(x[i],x[j])+coef0)+(i==j)*balanced_ridges[i];
	}
    ...
    double kernel_custom(int i, int j) const
	{
		...
		return Custom_Kernel[Custom_M*j2 +i2]+(i==j)*balanced_ridges[i];
	}
